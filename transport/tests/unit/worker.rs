...
// ── The driver: brought by the worker, or riding in the artifact ──

#[test]
fn a_node_that_asks_for_something_is_served_by_the_drivers_over_there() {
    // The catalog does not travel and neither does the driver: both are
    // supplied by whoever executes on each side. `ask` returns `Await` and only
    // finishes because the worker was stood up with `serve_with`.
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_test-worker"));
    cmd.arg("--driver");
    let w = Worker::spawn(cmd).unwrap();

    let g = graph_with(&["ask"], &[]);
    let c = catalog();
    let p = on_hosts(&[("ask", "worker1")]);

    // This side has no driver, and does not need one: the turns are spent there.
    assert_eq!(
        run(&g, &c, &p, &w, Value::Null).unwrap(),
        Value::text("HELLO")
    );
...
#[test]
fn a_driver_that_came_in_the_artifact_serves_what_the_nodes_ask_for() {
    // The other way it gets there, and the one the generic worker uses: an
    // empty worker has no driver of its own either, so the artifact brings it
    // alongside the nodes. Here an `adds` spec naming `shout` carries one.
    let w = Worker::spawn(empty())
        .unwrap()
        .carrying(adds("shout,x=5"), "rust");

    let mut c = Catalog::new();
    c.insert("ask", Arc::new(Add(1.0)));
    let g = graph_with(&["ask"], &[]);
    let p = on_hosts(&[("ask", "worker1")]);

    assert_eq!(
        run_provisioned(&g, &c, &p, &w, Value::Null).unwrap(),
        Value::text("HELLO")
    );
}

#[test]
fn an_artifact_without_a_driver_leaves_the_worker_without_one() {
    // The same worker and the same node, with an artifact that names none.
    let w = Worker::spawn(empty())
        .unwrap()
    ...
    let said = run_provisioned(&g, &c, &p, &w, Value::Null)
        .unwrap_err()
        .to_string();

    assert!(said.contains("no driver"), "the reason is missing: {said}");
}

#[test]
fn without_a_driver_over_there_the_failure_comes_back_saying_so() {
    // The other half: the same node on a worker stood up with plain `serve`.
    let g = graph_with(&["ask"], &[]);
    let c = catalog();
    let p = on_hosts(&[("ask", "worker1")]);
    let w = worker();
