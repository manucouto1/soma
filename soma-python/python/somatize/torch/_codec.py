...
crosses into a store is bytes, and `somatize_store` never finds out any of it
was ever a tensor. The frontier does not vanish, it moves — from *an opaque does
not travel* to *an opaque nobody registered a codec for does not travel*.

What it does **not** restore is the graph the tensor came from: what `load` gives
back is a **leaf**, and a backward pass stops there. That is exactly why a cached
prefix has to be settled.
"""
...
from somatize._somatize import codec
...
def register() -> None:
    """Says how a tensor is written down. Called on importing `somatize.torch`,
...
def dump(tensor: "torch.Tensor") -> bytes:
    ...
    nobody reads. Whoever trains across a cut says so itself.
    """
    buffer = io.BytesIO()
...
def load(raw: bytes) -> Any:
    ...
    Not where it was saved: a store is shared between machines, and one that only
    reads back on the box that wrote it is not shared at all. Whoever receives it
    moves it, which is what a placed node already does with its input.

    `weights_only=True` because unpickling arbitrary objects out of a shared
    store would make a cache into a way in.
    """
