...
half-updated repository executes other code and the result comes out different
without anyone noticing.

It covers the code with functionality and, transitively, whatever of yours that
code names — a helper in the same module, a base class, a module constant, a
class composed in `__init__`. It compares the **AST** and not the text, so a
comment or a docstring changes nothing.

It stops at **what is installed**, at **what has no source**, and at
**`somatize` itself**, whose version goes into the client's identity so that
upgrading the library does not invalidate every class at once.

What it does not cover, said before it bites: whatever the code **does not
name** — a data file opened by path, an environment variable — which is the
limit Bazel and Nix have too; and the **body of a decorator of your own**, since
the wrapper closes over the wrapped function rather than naming it. That one is
closable, and `test_a_decorator_of_your_own` goes green the day somebody does.
"""
...
def bill(cls: type) -> list[dict[str, Any]]:
    """**What the version was computed over**, said instead of only hashed: every
    class and function of yours the fingerprint reached, transitively, and every
    distribution it stopped at. Sorted, so two processes list it the same way.

    It exists because that walk answers a second question — **which files is this
    node made of?** A network written across four modules and assembled in one
    `__init__` is one class to `inspect.getsourcefile` and four files to whoever
    has to read it.
    ...
    An entry without a file is not a gap in the answer, it is where the
    fingerprint deliberately stops. Raises `CannotVersion` as `digest` does.
    """
    return sorted(
...
class _Reach:
    """What one walk found: what it hashes, and what it walked over. Two
    containers because they answer to different rules — `pieces` is a set of
    strings whose exact contents **are** the fingerprint, so anything added to it
    moves every version there is, while adding a field to `noted` costs nothing.
    """
...
def _written_where(obj: object) -> tuple[str, int, int] | None:
    """Where it is written: file, first line, how many. **Absolute**, as
    `inspect` gives it: making it relative needs a directory to be relative *to*,
    and the only one this could pick is the process's.
    """
    try:
...
def _names(code: types.CodeType) -> set[str]:
    """The global names this code and everything nested mention, descending into
    comprehensions, lambdas and inner functions.

    Read off the **instructions** and not off `co_names`, which mixes globals with
    attribute names: `self.model` puts `model` in there, and a module with a
    global called `model` had its value hashed into a class that never named it —
    and then a `--strict` worker refuses over a mismatch that does not exist.
    """
...
def _function_of(member: object) -> Callable[..., Any] | None:
    ...
    **unwrapped**. A decorator returns a function whose code mentions the
    decorator's globals and not the ones in the body it wraps.

    Not hypothetical: `Node.__init_subclass__` wraps every node's `__init__`, so
    every node reached this with a wrapper in hand and a class composed in
    `__init__` was left out of its fingerprint — the cache hits and hands back
    ...
    # decorator that does not use it stays opaque. And what it lands on is
    # checked rather than assumed: a node with no `__init__` of its own gets
    # `object.__init__` wrapped, a C slot with no `__code__` to read.
