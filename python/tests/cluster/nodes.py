...
# ── The spam case: preprocessing, embeddings, a classifier ──
#
# Three nodes that are a real pipeline and not a shape: text comes in, tokens
# come out of the first, a vector out of the second and a class out of the
# third. What makes them worth having here is **where** each can run.

VOCAB = 4096
"""How many buckets a word can hash into, `0` reserved for padding. Small on
purpose: an embedding table is `VOCAB x dim` and this one crosses a wire on
every configuration the study tries."""

WIDTH = 32
"""How many words of a message are kept. Cutting to a fixed width is what makes
a batch a **rectangle**, and a batch that is not one cannot be the seam of a
